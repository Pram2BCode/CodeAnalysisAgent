...
# REVIEW [CR005]: Missing docstring for function `addStudent`. -> SUGGESTION: Add a docstring describing the function's purpose, parameters, and return value.  Example: `def addStudent(studentId, name, age, enrollmentYear=DEFAULT_ENROLLMENT_YEAR):
    ...
    if len(studentId) > STUDENT_ID_LENGTH_LIMIT:
        ...
        return
        # REVIEW [CR002]: Printing API key -> SUGGESTION: Remove the print statement that reveals the API key.  Log sensitive information to a secure log location, not the console.
    ...
    # REVIEW [CR010]: Use of print() for logging and insecure logging of API key -> SUGGESTION: Replace print() with proper logging using the logging module.  Do not log sensitive information like API keys.
